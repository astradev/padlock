	...
	public function generateTree() {
        ...
        public function generateOptionTree() {
                ...
                while( ! empty( $tree ) ) {
                  ...
                  if( $currNode['depth'] < $currDepth ) {
                      $result .= str_repeat( '1', $currDepth - $currNode['depth'] );
                  }
                  $result .= '<option value="'. $currNode['id'] .'">'. $currNode['name'];
                  $currDepth = $currNode['depth'];
                  if( empty( $tree ) ) {
                    $result .= str_repeat( '<option><div class="divider"></div></option><option>Neuer Ordner</option>', $currDepth + 1);
                  }
                } 
                return $result;
        }
